                                    ...
                                    <div class="flex gap-x-5 max-md:w-full max-md:gap-x-5">
                                        ...
                                        <div class="grid gap-y-2.5 max-md:w-full max-md:gap-y-0">
                                            <div class="flex justify-between">
                                                <p class="text-base !leading-[1.625rem] font-medium max-md:text-sm">
                                                    @{{ item.product.name }}
                                                </p>
                ...
                methods: {
                    ...
                    remove(id) {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                this.$axios.delete(
                                        `{{ route('shop.api.customers.account.wishlist.destroy', '') }}/${id}`
                                    )
                                    .then(response => {
                                        this.wishlistItems = this.wishlistItems.filter(item => item
                                            .id != id);
                    ...
                    removeAll() {
                        this.$emitter.emit('open-confirm-modal', {
                            agree: () => {
                                ...
                                this.$axios.post(
                                        "{{ route('shop.api.customers.account.wishlist.destroy_all') }}", {
                                            '_method': 'DELETE',
                                        })
                                    .then(response => {
                                        this.wishlistItems = [];

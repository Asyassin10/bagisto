...
<v-media-images name="{{ $name }}" v-bind:allow-multiple="{{ $allowMultiple ? 'true' : 'false' }}"
...
@pushOnce('scripts')
 ...
 --}}
                            <p class="grid text-center text-sm font-semibold text-gray-600 dark:text-gray-300">
                                @lang('admin::app.components.media.images.add-csv-btn')
    ...
    <script type="module">
        app.component('v-media-images', {
            ...
            mounted() {
                ...
                for (let i = 0; i < arr.length; i++) {
                    arr[i].url = arr[i].url.replace("storage", "cache/small");
                }
                this.images = arr;
            },
            ...
            methods: {
                add() {
                    ...
                    console.log("imageInput")
                    const validFiles = Array.from(imageInput.files).every(file => file.type.includes('text/csv'));

                    if (!validFiles) {
                        this.$emitter.emit('add-flash', {
                            type: 'warning',
                            message: "Veuillez choisir un fichier csv"
                        });
        ...
        app.component('v-media-image-item', {
            ...
            mounted() {
                ...
                console.log("Component Props:", this.$props.image.file);

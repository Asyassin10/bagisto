...
class AdminSocieteController extends Controller
{
    ...
    public function deleteSociete($societe_id)
    {
     ...
     /**
     * Définir plusieurs sociétés comme partenaires ou non-partenaires du congrès
     */
    public function bulkSetPartner(Request $request)
    {
        $request->validate([
            'societe_ids' => 'required|json',
            'is_partner' => 'required|boolean'
        ]);

        $societeIds = json_decode($request->societe_ids, true);
        $isPartner = $request->is_partner;

        if (empty($societeIds) || !is_array($societeIds)) {
            return redirect()->back()->with('error', 'Aucune société sélectionnée.');
        }

        $updatedCount = 0;
        $productUpdatedCount = 0;

        foreach ($societeIds as $societeId) {
            $societe = Societe::find($societeId);
            
            if ($societe) {
                // Mettre à jour le statut partenaire de la société
                $societe->is_congrate_partner = $isPartner;
                $societe->save();
                $societe->refresh();
                $updatedCount++;
                ...
                if ($admin) {
                    ...
                    foreach ($products as $product) {
                        $product->is_congrate_partner = $isPartner;
                        $product->save();
                        $productUpdatedCount++;
                    }
                }
        ...
        $actionType = $isPartner ? 'partenaires congrès' : 'non-partenaires congrès';
        $message = "$updatedCount société(s) ont été définies comme $actionType avec succès.";
        
        if ($productUpdatedCount > 0) {
            $message .= " $productUpdatedCount produit(s) associé(s) ont également été mis à jour.";
        }

        return redirect()->route('admin::societe-admin.index')->with('success', $message);
    }

    /**
     * Toggle le statut partenaire congrès d'une société (méthode existante modifiée)
     */
